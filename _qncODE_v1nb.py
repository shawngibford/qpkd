...
from qnc import QuantumNeuralParameterEstimator # first attempt at quantum circuit for ODE
import pandas as pd
import numpy as np
import torch
...
class DiscreteNeuralParameterEstimator(nn.Module):
    """Deprecated: Classical MLP estimator has been removed in favor of the quantum estimator.

    Any attempt to instantiate this class will raise an error. Use
    `QuantumNeuralParameterEstimator` from `qnc.py` instead.
    """
    def __init__(self, *args, **kwargs):
        super().__init__()
        raise NotImplementedError(
            "DiscreteNeuralParameterEstimator is deprecated. "
            "Please use QuantumNeuralParameterEstimator from qnc.py."
        )
...
class DiscreteDosePKPDModel(nn.Module):
    ...
    def simulate_subject_discrete(self, subject_id, covariates, dose_schedule, simulation_times, baseline_biomarker=16.0):
        ...
        # IMPORTANT: Preserve autograd by calling the estimator's forward() directly
        # instead of predict_single_subject() (which uses torch.no_grad()).
...
def main_discrete():
    ...
    try:
        ...
        # quantum estimator instantiation
        # n_subjects, embedding_dim kept for interface compatibility (not used in quantum module)
        q_estimator = QuantumNeuralParameterEstimator(n_subjects=n_subjects, device="cpu")
        q_estimator.to(device)
        # Section 3: Discrete Dosing Mechanistic Model
        pkpd_model = DiscreteDosePKPDModel(q_estimator, device=device) #param_estimator replaced with q_estimator
...
def test_discrete_dosing():
    ...
    #param_estimator.to(device)
    # Create minal quntum set-up
    q_estimator = QuantumNeuralParameterEstimator(n_subjects=10, device="cpu")
    q_estimator.to(device)
    model = DiscreteDosePKPDModel(q_estimator, device=device)
    model.to(device)
...
# ============================================================================
# SECTION 5: UTILITIES - CIRCUIT DRAWING, GRADIENT CHECK, SANITY TRAINING
# =========================================================================
def draw_quantum_circuit(model, show_plot: bool = False, save_path: str | None = None):
    """Draw the quantum circuit via the estimator's visualize_circuit if available."""
    if hasattr(model, "param_estimator") and hasattr(model.param_estimator, "visualize_circuit"):
        try:
            return model.param_estimator.visualize_circuit(save_path=save_path, show_plot=show_plot)
        except Exception as e:
            print(f"Circuit drawing failed: {e}")
    else:
        print("Model's parameter estimator does not support visualize_circuit().")
    return ""


def gradient_check(model: DiscreteDosePKPDModel, data_processor: DataProcessor, subject_id: int) -> bool:
    """Run a quick gradient check to ensure autograd flows into quantum parameters."""
    model.train()
    subject = data_processor.get_subject_data(subject_id)
    dosing = subject['dosing_data']
    if len(dosing) == 0:
        print(f"Subject {subject_id} has no dosing; pick a subject with dosing.")
        return False

    covariates = subject['covariates']
    dose_schedule = [(float(row['TIME']), float(row['AMT'])) for _, row in dosing.iterrows()]
    obs_times = sorted(set(subject['pk_data']['TIME'].values.tolist() + subject['pd_data']['TIME'].values.tolist()))
    if not obs_times:
        print(f"Subject {subject_id} has no observations.")
        return False
    sim_times = torch.tensor(obs_times, dtype=torch.float32, device=model.device)

    solution, pk_params, pd_params = model.simulate_subject_discrete(
        subject_id, covariates, dose_schedule, sim_times, baseline_biomarker=16.0
    )

    loss = pk_params.sum() + pd_params.sum()
    model.param_estimator.zero_grad(set_to_none=True)
    loss.backward()
    has_grad = any(p.grad is not None and torch.isfinite(p.grad).all() and p.grad.abs().sum() > 0 for p in model.param_estimator.parameters())
    print(f"Gradient flows into quantum parameters: {has_grad}")
    return has_grad
...
def run_sanity_training(model: DiscreteDosePKPDModel, trainer: DiscretePKPDTrainer, max_subjects: int = 3, epochs: int = 3):
    """Run a short training loop on a subset to validate decreasing loss and parameter updates."""
    train_subjects = trainer.data_processor.train_subjects[:max_subjects]
    if not train_subjects:
        print("No training subjects available.")
        return

    print(f"Running sanity training: subjects={len(train_subjects)}, epochs={epochs}")
    before = _param_norm(model.param_estimator)
    print(f"Param L2 norm (before): {before:.6f}")

    for ep in range(1, epochs + 1):
        loss = trainer.train_epoch(train_subjects)
        print(f"Epoch {ep}: loss={loss:.6f}")

    after = _param_norm(model.param_estimator)
    print(f"Param L2 norm (after):  {after:.6f}")
    print(f"Parameters updated: {abs(after - before) > 1e-9}")
if __name__ == "__main__":
    ...
    if test_success:
        ...
        # Draw the quantum circuit (text + optional matplotlib)
        print("\nDrawing quantum circuit (text-only; set show_plot=True to display)...")
        _ = draw_quantum_circuit(pkpd_model, show_plot=False)

        # Gradient check on one subject (with dosing)
        print("\nRunning gradient check on first training subject (with dosing, if available)...")
        candidate = None
        for sid in trainer.data_processor.train_subjects:
            if len(trainer.data_processor.dosing_data[trainer.data_processor.dosing_data['ID']==sid]) > 0:
                candidate = sid
                break
        if candidate is not None:
            _ = gradient_check(pkpd_model, data_processor, candidate)
        else:
            print("No training subjects with dosing found for gradient check.")

        # Short sanity training loop
        print("\nRunning short sanity training loop...")
        run_sanity_training(pkpd_model, trainer, max_subjects=3, epochs=3)
    else:
